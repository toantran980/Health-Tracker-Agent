...
from api.external_apis import (
    search_food_by_name, get_food_by_barcode,
    log_natural_language_meal, search_exercise, get_weather_context
)
...
# Paste these routes into your existing routes.py
# Add to imports at top:
#   from ai_modules.chatbot import HealthChatbot, UserHealthSnapshot
#   from api.external_apis import search_food_by_name, get_food_by_barcode, log_natural_language_meal, search_exercise, get_weather_context
#   bot_sessions: Dict[str, HealthChatbot] = {}
...
def chat(user_id):
    ...
    # Build snapshot from existing user object + any live data passed in
...
def reset_chat(user_id):
    """Reset the chatbot conversation history for a user"""
...
def food_search():
    """Search foods by name via Open Food Facts (no key needed)"""
    query = request.args.get("q", "").strip()
    if not query:
        return jsonify({"error": "q parameter is required"}), 400

    results = search_food_by_name(query, page_size=request.args.get("limit", 5, type=int))
    return jsonify({"results": results, "count": len(results)}), 200


@app.route('/api/food/barcode/<barcode>', methods=['GET'])
def food_by_barcode(barcode):
    """Lookup food nutrition by barcode via Open Food Facts"""
...
@app.route('/api/food/log-text', methods=['POST'])
def food_log_text():
    """
    Log a meal from plain text via Nutritionix.
    ...
    try:
        results = log_natural_language_meal(text)
        return jsonify({"foods": results, "count": len(results)}), 200
...
def exercise_search():
    """Search exercise database via Wger (no key needed)"""
    query = request.args.get("q", "").strip()
    if not query:
        return jsonify({"error": "q parameter is required"}), 400
...
def weather_context():
    """
    Get weather-based wellness hints via Open-Meteo (no key needed).
    ...
    result = get_weather_context(lat, lon)
    return jsonify(result), 200
