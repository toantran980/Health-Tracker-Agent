"""Main application entry point"""
import sys
import logging

from api.routes import app

# Configure logging
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger(__name__)

def main():
    """Run the application"""
    logger.info("Starting AI Health & Wellness Tracker")
    logger.info("Initializing Flask API server...")

    import config
    port = config.PORT
    host = config.HOST
    debug = config.DEBUG

    logger.info(f"Server will run at: http://{host}:{port}")
    logger.info("Available endpoints:")
    logger.info("  POST   /api/user/create")
    logger.info("  GET    /api/user/<user_id>")
    logger.info("  POST   /api/user/<user_id>/password")
    logger.info("  POST   /api/auth/login")
    logger.info("  POST   /api/auth/logout")
    logger.info("  GET    /api/auth/me")
    logger.info("  GET    /api/health")
    logger.info("  GET    /api/insights/<user_id>")
    logger.info("  POST   /api/meals/log")
    logger.info("  GET    /api/activity-recommendations/<user_id>")
    logger.info("  POST   /api/activity/log")
    logger.info("  GET    /api/activity/logs/<user_id>")
    logger.info("  GET    /api/activity/trends/<user_id>")
    logger.info("  GET    /api/nutrition/analysis/<user_id>")
    logger.info("  GET    /api/nutrition/meal-recommendations/<user_id>")
    logger.info("  GET    /api/nutrition/recommendations/<user_id>")
    logger.info("  GET    /api/schedule/available-slots/<user_id>")
    logger.info("  POST   /api/schedule/optimize/<user_id>")
    logger.info("  GET    /api/schedule/history/<user_id>")
    logger.info("  POST   /api/productivity/predict/<user_id>")
    logger.info("  GET    /api/productivity/sessions/<user_id>")
    logger.info("  GET    /api/productivity/optimal-time/<user_id>")
    logger.info("  GET    /api/food/search")
    logger.info("  GET    /api/food/barcode/<barcode>")
    logger.info("  GET    /api/exercise/search")
    logger.info("  GET    /api/wger/<endpoint>")
    logger.info("  GET    /api/exercisedb/search")
    logger.info("  POST   /api/chat/<user_id>")
    logger.info("  POST   /api/chat/<user_id>/reset")

    try:
        app.run(debug=debug, host=host, port=port, use_reloader=False)
    except Exception as e:
        logger.error(f"Failed to start server: {e}")
        sys.exit(1)

if __name__ == '__main__':
    main()
